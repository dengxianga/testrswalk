...
JointValues Walk2014Generator::makeJoints(ActionCommand::All* request,
        ...
        // Penn ---------------------
        std::cout<<"WalkGenerator makeJoints walk2014Option "<<walk2014Option<<std::endl;
        std::cout<<"walkState "<<walkState <<std::endl;
        std::cout<<"request->body.actionType "<<request->body.actionType <<std::endl;
        std::cout<<"t "<<t<<" dt "<<dt<<std::endl;
        std::cout<<"globalTime "<<globalTime<<std::endl;
        std::cout<<"nextFootSwitchT "<<nextFootSwitchT<<std::endl;
...
//comment this out so we never do rUNSWift slow kick
                ...
                if (abs(hiph - STAND_HIP_HEIGHT) < .0001) {     // and robot has reached stand height
                        ...
                        std::cout<<"walk2014Option = STANDUP \n ";
                }
                ...
                if (abs(hiph - WALK_HIP_HEIGHT) < .0001 || fakewalk) {        // and we are at the designated walking height
                        ...
                        walk2014Option = CROUCH;         // continue crouching
                        std::cout<<"walk2014Option = CROUCH2 \n ";
                }
        }
...
// 4. Execute Walk2014 Option
        ...
        if (walk2014Option == WALK and nextFootSwitchT > 0) {
                std::cout<<"WalkGenerator WALK next foot!"<<std::endl;
...
//   std::cout << t << endl;
        ...
        // Penn -------------------------
        if ((t > 0.75*T && (bodyModel.lastZMPL*bodyModel.ZMPL < 0.0 || (fakewalk && t>=T) )) || t > 3*T) supportFoothasChanged = true;
        std::cout<<"supportFoothasChanged "<<supportFoothasChanged<<std::endl;
        bool isLeftphase=bodyModel.ZMPL < 0 || (fakewalk && (steps % 2) ==0);
        if (supportFoothasChanged && walk2014Option == WALK) {
          steps++;
        }
        if (supportFoothasChanged) bodyModel.setIsLeftPhase(isLeftphase);  // set isLeft phase in body model for kinematics etc
        // =======================================
